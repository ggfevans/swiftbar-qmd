...
3. Click each Global action and confirm it spawns a `qmd …` child + writes a PID file under `~/.cache/qmd-swiftbar/jobs/` + log file under `~/.cache/qmd-swiftbar/logs/`. Confirm the action row rewrites to `Running: <verb>… (<Nm>) … disabled=true` while in flight.
...
7. Edit `~/.config/qmd-swiftbar/config.yml` to introduce a deliberate range violation (e.g. `coverage.amber_percent: 999`); verify the menu shows the `⚠ Config error — see logs` header and `Show last error` footer.
...
2. Merge to `main`.
3. `git tag -a v1.0.0 -m "qmd-swiftbar v1.0.0"` on the merged commit.
